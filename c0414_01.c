	...
	while (1) {
		...
		switch (choice) {
			case 1 : // 추가하기
				...
				break;
			case 2 : // 수정하기
				printf("수정하실 기존 상품명 :"); scanf_s("%s", temp, sizeof(temp));
				isDup = 0;
				for (int i = 0; i < cnt; i++) {
					if (!strcmp(temp, arName[i])) {
						isDup = 1;
				...
				if (isDup) {
					...
					if(!isDup) {                          // &arPrice[foundIdx]
						printf("상품 가격 :"); scanf_s("%d", arPrice + foundIdx);
				...
				break;
			case 3 : // 검색하기
				printf("검색하실 상품명 입력 :"); scanf_s("%s", temp, sizeof(temp));
				isDup = 0;
				for (int i = 0; i < cnt; i++) {
					if (!strcmp(temp, arName[i])) {
						printf("%s(%d원) \n", arName[i], arPrice[i]);
				...
				break;
			case 4 : // 삭제하기
				printf("삭제하실 상품명 입력 :"); scanf_s("%s", temp, sizeof(temp));
				isDup = 0;
				for (int i = 0; i < cnt; i++) {
					if (!strcmp(temp, arName[i])) {
						printf("%s(%d원) \n", arName[i], arPrice[i]);
